...
class NatSLU(Model):
    ...
    def batch_process(self, lines):
        ...
        try:
            ...
            # print(seq_in[0])
            sentence_length = len(seq_in[0].split(' '))
            # print(f'sentence length = {sentence_length}')
            slots_length = 0
            # print(seq_out[0].split(' '))
            for i in seq_out[0].split(' '):
                if i != 'o' and i != 'O':
                    slots_length += 1
            # print(f'slot length = {slots_length}')
        except:
            print(lines)
            print('input data is unvalid!')
        ...
        # print(f'seq_in_ids = {seq_in_ids[0].shape}, seq_out_ids = {seq_out_ids[0].shape}')
    ...
    def evaluation(self, sess, location='none', batch_size=1000):
        """Do Evaluation"""
        ...
        if location == 'none':
            ...
            print(location)
            print('for own location of data ')
            valid_path = location
        ...
        while 1:
            ...
            step = step + 1
            batch, iterator, last_batch = self.get_batch_np(batch_iter, valid_path, batch_size)
            batch_iter = iterator
    ...
    def inference(self, sess, epoch, diff, dump):
        ...
        def post_process(outputs):
            ...
            ref = []
            pred = []
            # print(type(input_data), type(correct_slot), type(correct_intent), type(pred_intent), type(pred_slot))
            # print(input_data.shape, correct_slot.shape, correct_intent.shape, pred_intent.shape, pred_slot.shape)
    ...
    def fit(self, sess):
        ...
        print(f'restore = {self.arg.restore}')
        if self.arg.restore:
    ...
    def prediction(self, sess, model_path: str, location='none', batch_size: int = 1):
        """Do Prediction"""
        self.saver = tf.train.Saver()
        # self.saver.restore(sess, self.save_path)
        ...
        def valid(eval_outputs):
            ...
            f1, precision, recall = local_utils.computeF1Score(correct_slot_label, pred_slot_label)
            # print("F1: {}, precision: {}, recall: {}".format(f1, precision, recall)) print( f'pred intent = {
            # pred_intent}, correct_intent = {correct_intent}, pred_slot = {pred_slot}, correct_slot = {
            # correct_slot}, ')
            print(f1, slot_acc, intent_acc, sent_acc)

            ref = []
            pred = []

            input_data = eval_outputs[5]
            # print(type(input_data), type(correct_slot), type(correct_intent), type(pred_intent), type(pred_slot))
            # print(input_data.shape, correct_slot.shape, np.array(correct_intent).reshape(1,).shape, pred_intent.shape, pred_slot.shape)
            for words, c_i, p_i, seq_len, c_slot, p_slot in zip(input_data, np.array(correct_intent).reshape(1, ),
                                                                pred_intent, sequence_length, correct_slot, pred_slot):
                words_output = ' '.join(
                    [self.seq_in_tokenizer.index_word[idx] for idx, _ in zip(words, range(seq_len))])
                c_i_output = self.label_tokenizer.index_word[c_i]
                c_slot_output = ' '.join(
            ...
            # print(f'REF = {ref}, \nPRED = {pred}')
            return f1, slot_acc, intent_acc, sent_acc, ref, pred
        ...
        while 1:
            ...
            try:
                # first pass
                print(seq_in_ids)
                eval_outputs = sess.run(self.test_outputs, feed_dict={self.input_data: seq_in_ids,
                                                                      self.input_tags: first_pass_in_tags,
                                                                      self.sequence_length: sequence_length,
                                                                      self.slots: seq_out_ids,
                                                                      self.intent: label_ids})
                ...
                break
            end_time = time.time()
            print(f'time_taken for one prediction = {end_time - start_time}')
            time_taken.append(end_time - start_time)

            f1_batch, slot_acc_batch, intent_acc_batch, sent_acc_batch, reference, prediction = valid(eval_outputs)
        ...
        average_time /= len(time_taken)
        print(f'Average time for prediction = {average_time}')
        return
...
def get_arguments():
    ...
    # Transformer
    parser.add_argument("--hidden_size", type=int, default=96, help="hidden_size")
    parser.add_argument("--filter_size", type=int, default=96, help="filter_size")
    ...
    parser.add_argument('--attention_dropout', default=0.05, type=float, help='att_dropout')
    ...
    parser.add_argument('--job', dest="job", default='training', help='select jobs from training and prediction')
...
def train_and_save():
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.job == 'training':
            ...
            saver.save(sess, f'./model_{args.dataset}_{args.max_epochs}/model_{args.dataset}_{args.max_epochs}')
            endtime = time.time()
            print(endtime-starttime)
            print('Model Trained Successfully and saved to directory')
        elif args.job == 'prediction':
            model.prediction(sess, location='data/snips/valid/data_check', batch_size=1,
                             model_path='model_atis_250/model_atis_250')
            print("prediction completed..")
        else:
            print("None of the jobs selected [prediction, training]")
